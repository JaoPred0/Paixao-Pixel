    ...
    >
      <div className="max-w-7xl mx-auto px-6 md:px-10">
        <div className="flex justify-between items-center py-4 md:py-3">
          {/* === LOGO === */}
          <RouterLink to="/" className="flex items-center gap-2 select-none cursor-pointer">
            <div className="flex items-center justify-center p-2 rounded-xl shadow-md">
              <img
                src="/logo.png"
                alt="Paixão Pixel Logo"
          ...
          <div className="hidden md:flex items-center space-x-10 text-gray-700 font-medium">
            {links.map((link, idx) => {
              // Se o href começa com "/", usa RouterLink; caso contrário, ScrollLink
              const isRoute = link.href.startsWith("/");
              return isRoute ? (
                <RouterLink
                  key={idx}
                  to={link.href}
                  className="relative cursor-pointer group transition-all duration-300 hover:text-pink-600"
                >
          ...
          {links.map((link, idx) => {
            ...
            return isRoute ? (
              <RouterLink
                ...
                className="px-6 py-3 text-gray-700 font-medium hover:text-pink-600 border-b border-pink-100 transition-colors duration-300"
              >
                {link.label}
              </RouterLink>
            ) : (
